...
for root, dirs, files in os.walk('app/(main)/settings'):
    for file in files:
        if file.endswith('.tsx'):
            process_file(os.path.join(root, file))

for root, dirs, files in os.walk('app/(main)/(tabs)'):
    for file in files:
        if file.endswith('.tsx'):
            process_file(os.path.join(root, file))

process_file('src/modules/timeline/components/HorizontalDateNavigator.tsx')
