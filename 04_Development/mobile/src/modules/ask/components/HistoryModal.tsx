...
export const HistoryModal = forwardRef<HistoryModalRef, HistoryModalProps>(({ onSelectConversation, onNewChat }, ref) => {
 ...
 const result = [];
 if (pinned.length > 0) result.push({ title: t('ask.pinned', 'Pinned'), data: pinned });
 if (today.length > 0) result.push({ title: t('ask.today', 'Today'), data: today });
 if (yesterday.length > 0) result.push({ title: t('ask.yesterday', 'Yesterday'), data: yesterday });
 if (older.length > 0) result.push({ title: t('ask.older', 'Previous 30 Days'), data: older });
 ...
 <Text style={styles.title}>{t('ask.history', 'History')}</Text>
 <TouchableOpacity onPress={handleCreateNew} style={styles.newButton}>
 <MaterialIcons name="add" size={20} color={colors['on-primary']} />
 <Text style={styles.newButtonText}>{t('ask.newChat', 'New Chat')}</Text>
 </TouchableOpacity>
 </View>

 {isLoading ? (
 <View style={styles.loadingContainer}>
 <Text style={styles.loadingText}>{t('ask.loadingHistory', 'Loading history...')}</Text>
 </View>
 ) : (
 <SectionList
 sections={sections}
 ...
 <Text style={styles.emptyText}>{t('ask.noHistory', 'No conversation history.')}</Text>
 </View>
 }
 />
 )}
