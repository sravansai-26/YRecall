...
export function AskScreen() {
 ...
 let errorMessage = t('ask.errorSend', 'Failed to send message.');
 if (error.response) {
 if (error.response.status >= 500) {
 errorMessage = `HTTP ${error.response.status}\n\n${error.response.data?.detail || t('ask.serverError', 'Server Error')}\n\n${t('ask.connectionClosed', 'Supabase connection closed or backend failure.')}`;
 } else {
 errorMessage = `HTTP ${error.response.status}\n\n${error.response.data?.detail || t('ask.apiError', 'API Error')}`;
 }
 } else if (error.request) {
 errorMessage = t('ask.networkError', 'Network Error. Could not reach the backend.');
 } else {
 errorMessage = error.message;
 }
 console.error('[AskAI] Error:', error);
 require('react-native').Alert.alert(t('common.error', 'Error'), errorMessage);
 }
 });
 };
