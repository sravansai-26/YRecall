...
export const Composer = forwardRef<ComposerRef, ComposerProps>(({ onSend, isPending }, ref) => {
 ...
 require('react-native').Alert.alert(t('common.error', 'Error'), t('ask.startRecordFail', 'Failed to start recording.'));
 }
 };
 ...
 require('react-native').Alert.alert(t('common.fileTooLarge', 'File too large'), t('ask.audioUnder10mb', 'Audio recording must be under 10MB.'));
 setRecordingDuration(0);
 return;
 }
 ...
 require('react-native').Alert.alert(t('ask.transcribeFailed', 'Transcription Failed'), t('ask.transcribeRetry', 'Could not transcribe audio. Please try again.'));
 } finally {
 ...
 require('react-native').Alert.alert(t('common.uploadFailed', 'Upload Failed'), t('ask.uploadAudioFailed', 'Failed to upload audio attachment.'));
 }
 }
 } catch (err) {
 ...
 require('react-native').Alert.alert(t('common.uploading', 'Uploading'), t('ask.waitUploads', 'Please wait for all attachments to finish uploading.'));
 return;
 }
 ...
 setAttachments(prev => prev.filter(a => a.id !== tempId));
 require('react-native').Alert.alert(t('common.error', 'Error'), t('ask.uploadAttachFailed', 'Failed to upload attachment.'));
 }
 }
 };
 ...
 placeholder={t('ask.composerPlaceholder', 'Message YRecall...')}
 ...
 {isProcessingAudio ? (
 <Text style={styles.recordingText}>{t('ask.processing', 'Processing...')}</Text>
 ) : (
 <>
 <View style={styles.recordingDot} />
 <Text style={styles.recordingText}>
 {t('ask.recording', 'Recording')} {formatDuration(recordingDuration)} / 2:00
 </Text>
