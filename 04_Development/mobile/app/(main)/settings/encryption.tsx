...
export default function DataProtectionCenter() {
 ...
 className={`px-4 py-2 rounded-full ${value === opt.toLowerCase() ? 'bg-primary border-primary' : 'bg-surface-container-lowest '}`}
 >
 ...
 <View className="bg-surface-container-lowest p-4 rounded-2xl flex-row flex-wrap justify-between gap-y-4">
 {[
 ...
 <View key={item.name} className="w-[48%] bg-surface-container-low p-3 rounded-xl ">
 ...
 className={`px-4 py-2 rounded-full flex-row items-center gap-1 ${form.enhanced_protection_categories?.includes(cat.toLowerCase()) ? 'bg-secondary-container border-secondary-container' : 'bg-surface '}`}
 >
