...
export default function AutomationCenterScreen() {
  ...
  return (
    <Screen scrollable={false}>
      {/* Dynamic Header */}
      <View className="bg-surface z-50 h-16 w-full flex-row items-center justify-between px-margin-mobile border-b border-outline-variant/30">
        <View className="flex-row items-center">
          <TouchableOpacity onPress={() => router.back()} className="w-10 h-10 items-center justify-center -ml-2 rounded-full">
            <MaterialIcons name="arrow-back" size={24} color={colors.primary} />
          </TouchableOpacity>
          <Text className="font-title-lg font-bold text-on-surface ml-2">Operations Hub</Text>
        </View>
        ...
        <View className="flex-row items-center bg-primary/10 px-3 py-1.5 rounded-full border border-primary/20">
          ...
          <Text className="text-primary font-bold text-xs ml-1.5 tracking-wider">AI ACTIVE</Text>
        </View>
      </View>
      ...
      <ScrollView className="flex-1 bg-surface-container-lowest" contentContainerStyle={{ paddingBottom: 120 }}>
        ...
        <View style={styles.gradientHeader} className="bg-primary-container/30">
          <View className="px-margin-mobile pt-6 pb-2">
            ...
            <Text className="font-body-md text-on-surface-variant mb-6">
              The Automation Center processes your captures in the background to build your knowledge graph, summarize meetings, and prepare your daily briefs without any manual effort.
            </Text>

            <View className="flex-row flex-wrap gap-3">
              <View className="flex-1 min-w-[100px] bg-surface/80 p-4 rounded-3xl border border-outline-variant/20 shadow-sm items-center justify-center">
                ...
                <Text className="text-[10px] text-on-surface-variant uppercase font-bold mt-1 text-center">Actions Taken This Week</Text>
              </View>
              ...
              <View className="flex-1 min-w-[100px] bg-surface/80 p-4 rounded-3xl border border-outline-variant/20 shadow-sm items-center justify-center">
                ...
                <Text className="text-[10px] text-on-surface-variant uppercase font-bold mt-1 text-center">Active Workflows</Text>
              </View>
              ...
              <View className="flex-1 min-w-[100px] bg-surface/80 p-4 rounded-3xl border border-outline-variant/20 shadow-sm items-center justify-center">
                ...
                <Text className="text-[10px] text-on-surface-variant uppercase font-bold mt-1 text-center">Success Rate</Text>
              </View>
            </View>
          </View>
        </View>
        ...
        <View className="px-margin-mobile mt-6">
          ...
          {/* Clarification: Workflows are the background rules. Tasks are action items the AI has extracted for YOU to review. */}
          <View className="mb-8 animate-fade-in">
            <View className="flex-row justify-between items-center mb-4">
              <View className="flex-row items-center gap-2">
                ...
                <Text className="font-title-md text-on-surface font-bold">Action Required</Text>
              </View>
            ...
            <Text className="font-body-sm text-on-surface-variant mb-4 -mt-2">
              Tasks the AI has extracted for you or scheduled for your review.
            </Text>

            {loadingReminders ? <ActivityIndicator color={colors.primary} /> : (!reminders || reminders.length === 0) ? (
              <View className="bg-surface-container rounded-3xl p-6 items-center border border-outline-variant/20 shadow-sm">
                <MaterialCommunityIcons name="check-all" size={32} color={colors.outline} className="mb-2 opacity-50" />
                <Text className="text-sm font-bold text-on-surface-variant">You're all caught up.</Text>
                <Text className="text-xs text-on-surface-variant mt-1">No pending actions required.</Text>
              </View>
            ) : (
              reminders.slice(0, 3).map((reminder) => (
                <View key={reminder.id} className="bg-surface-container rounded-2xl p-4 flex-row items-start border border-outline-variant/30 mb-3 shadow-sm">
                  <TouchableOpacity 
          ...
          <View className="mb-8 animate-fade-in">
            <View className="flex-row justify-between items-center mb-4">
              <View className="flex-row items-center gap-2">
                ...
                <Text className="font-title-md text-on-surface font-bold">Background Workflows</Text>
              </View>
              <TouchableOpacity 
                onPress={() => router.push('/automation/create')} 
                className="bg-primary/10 px-3 py-1.5 rounded-full flex-row items-center gap-1 border border-primary/20"
              >
                <MaterialIcons name="add" size={16} color={colors.primary} />
                <Text className="text-primary font-bold text-xs uppercase tracking-wider">New</Text>
              </TouchableOpacity>
            </View>
            <Text className="font-body-sm text-on-surface-variant mb-4 -mt-2">
              The rules that govern how AI processes your incoming data.
            </Text>

            {loadingWorkflows ? <ActivityIndicator color={colors.primary} /> : (!workflows || workflows.length === 0) ? (
               <View className="bg-surface-container rounded-3xl p-8 items-center border border-outline-variant/20 shadow-sm">
              ...
              workflows.map((wf) => (
                <View key={wf.id} className={`bg-surface-container-lowest rounded-2xl p-4 border shadow-sm mb-3 ${wf.is_active ? 'border-primary/30 bg-primary/5' : 'border-outline-variant/20'}`}>
                  <View className="flex-row justify-between items-start">
                    ...
                    <View className="flex-row items-center gap-2">
                      ...
                      <TouchableOpacity 
                        ...
                        className="w-8 h-8 rounded-full bg-error/10 items-center justify-center border border-error/20"
                      >
                        <MaterialIcons name="delete-outline" size={16} color={colors.error} />
                      </TouchableOpacity>
                    </View>
                  </View>
                  <View className="flex-row mt-3 gap-2 border-t border-outline-variant/20 pt-3">
          ...
          <View className="mb-4 animate-fade-in">
            <View className="flex-row items-center gap-2 mb-4">
              ...
              <Text className="font-title-md text-on-surface font-bold">Live Activity Stream</Text>
            </View>
            <Text className="font-body-sm text-on-surface-variant mb-4 -mt-2">
              A real-time log of what the AI is executing right now.
            </Text>
            ...
            {loadingExecutions ? <ActivityIndicator color={colors.primary} /> : (!executions || executions.length === 0) ? (
              ...
              <View className="border-l-2 border-outline-variant/30 ml-4 pl-4 py-2 gap-6">
                {executions.slice(0, 5).map((exec) => (
                  <View key={exec.id} className="relative">
                    ...
                    {exec.logs && exec.logs.length > 0 && (
                      <View className="bg-surface-container p-2 rounded-xl mt-1 border border-outline-variant/20">
                        {exec.logs.map((l, idx) => (
